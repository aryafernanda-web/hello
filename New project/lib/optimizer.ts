...
import snapPointOnLine from "@turf/nearest-point-on-line";
...
function buildCableRouteOverlay(params: {
  ...
  const sliced = lineSliceAlong(cableLine, startMeters, endMeters, { units: "meters" }) as Feature<LineString>;
  const geometry =
...
async function evaluateCandidate(params: {
  ...
  if (canUseCable) {
    const snappedDp = snapPointOnLine(sharedCableLine, toTurfPoint(dp.position), { units: "meters" });
    const dpOffLine =
